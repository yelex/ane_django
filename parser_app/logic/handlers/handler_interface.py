...
from parser_app.logic.handlers.handler_tools import ParsedProduct, get_empty_handler_DF, validate_ParsedProduct, \
    get_simple_proxy, postprocess_parsed_product
...
class HandlerInterface:
    ...
    def _create_webdriver(self):
        ...
        proxy = get_simple_proxy(just_one=True)
        # proxy = '139.162.166.141:3128'
        print(f'{self._get_handler_name()} will use proxy : {proxy}')
        chrome_options.add_argument(f'--proxy-server={proxy}')
        self._driver = webdriver.Chrome(executable_path=Global().path_chromedriver, options=chrome_options)
    ...
    def extract_products(self) -> pd.DataFrame:
        ...
        try:
            print(f'Start create df by url list {self._get_handler_name()}')
    ...
    def _add_df_row_from_parsed_product(self, parsed_product: ParsedProduct, category_row) -> None:
        ...
        self._returned_df = self._returned_df.append({
                ...
                'site_code': self._get_handler_name(),
            },
            ignore_index=True,
        )
    ...
    def _update_category(self, parsed_item: ParsedProduct, category_row):
        ...
        for _, row in self._old_urls[self._old_urls['cat_title'] == category_row['cat_title']].iterrows():
    ...
    def _get_df_from_url_list(self) -> None:
        for _, url_row in self._old_urls.iterrows():
            ...
            try:
                ...
                print(f'in parser {self._get_handler_name()} Error during parsing single item:')
